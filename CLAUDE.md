...
- `src/tools/macro/sources/` — data adapters: yahoo-macro (FX/ETF), bi (BI website scraper + hedging compliance), bps (BPS API), imf (IMF Data API), bloomberg (REST proxy), refinitiv (RDP OAuth2), pertamina (domestic fuel prices + cost recovery formula, Kepmen ESDM 245/2022), x-social (X API v2 real-time social feed), political-risk-terms (shared keyword constants for Exa + X scoring)
