...
Tracks Indonesia's domestic political and social stability. Two channels: (A) social contract stress — sembako unaffordable + unemployment → Prabowo approval erosion → policy risk; (B) governance — authoritarian drift, investor confidence → CDS/IDR risk premium. Components: BPS unemployment rate (quarterly, TE scrape, normal 4.8%, stress 6.5%); Exa news sentiment across 3 signals (food_pressure, social_unrest, political_stability — keyword-scored, no LLM call); **X (Twitter) API v2 real-time social feed** (Bearer Token auth, 20 tweets/call, minute-zero unrest detection before Exa news publishes); seasonal context detection (Iduladha, Lebaran windows apply 30% discount to food stress). Blending: `socialUnrestComponent = max(exaScore, xScore × 0.85)` — X raises floor with 15% noise discount, never lowers Exa. X score stored as `political_x_social_score` in DB. Political Risk Index 0-100. Sources: `sources/political-risk.ts` (Exa), `sources/x-social.ts` (X API v2), `sources/political-risk-terms.ts` (shared keyword constants for both). Requires EXASEARCH_API_KEY for Exa; X_BEARER_TOKEN for X feed; degrades gracefully to unemployment-only if both absent. Silent Crisis Detector weight: 0.05.
