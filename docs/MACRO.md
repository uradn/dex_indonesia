...
**M11 — Domestic Pressure Engine** (`domestic_pressure_engine`): 10 PIHPS food commodities (beras medium, cabai merah/rawit, bawang merah/putih, daging sapi/ayam, telur, minyak goreng curah, gula). Food = ~30% CPI. Food Stress Index 0-100 via 90d z-score. Alert when ≥2 commodities z>1.5. Food CPI vs APBN implied 3.75%. **BBM subsidy gap:** Pertalite IDR 10,000/L (Kepmen ESDM 245.K) vs cost recovery `(max(0,Brent−russiaDiscount)/158.987)×USDIDR×1.40`; stores `bbm_cost_recovery_idr_liter`, `bbm_subsidy_gap_idr_liter`; >2k Y, >4k O, >7k R. **Russia deal caveat:** formula pure Brent-based by default — overstates gap by ~Rp485–975/L at 25–50% ESPO supply share; set `RUSSIA_CRUDE_DISCOUNT_USD` env var (blended $/bbl) when bulk delivery confirmed. **ICP threshold:** Brent vs $100/bbl (Bahlil Apr 2026); <$80 G, 80-90 Y, 90-100 O, >100 R. Hormuz flag at Brent >$90. Env overrides: `PERTALITE_PRICE_IDR`, `RUSSIA_CRUDE_DISCOUNT_USD`. Chain: food+BBM → CPI → BI hike → SBN → outflow. Sources: hargapangan.id Playwright (daily) → TE meta scrape (fallback), `sources/pertamina.ts`. SCD 0.06.
...
**APBN 2026 macro constants** (UU No.17/2025): USDIDR 16,500 | ICP $70/bbl | GDP growth 5.4% | CPI 2.5% | SBN 10Y 6.9% | Revenue 3,153.6T | Spending 3,842.7T | Deficit 2.68% GDP | Post-efisiensi spending ~3,534.7T | GDP 25,714.2T. BI Rate as of 9 June 2026: 5.50% (+25bps from 5.25%) — **inter-cycle hike via weekly RDG**; **hike lanjutan Jul 2026: 5.75%** (+25bps, DFR 4.75%, LF 6.50%); dipertahankan Jul & Agu RDG. Term premium ~1.21% (SBN 6.96% − BI Rate 5.75%). Menkeu: Suahasil Nazara (dilantik 14 Sep 2026, Keppres 97p/2026). BI Gov: Destry Damayanti (dilantik 2 Sep 2026, Keppres 92/P/2026). **Russia energy deal (Sep 2026):** framework MoU ~150 juta barel ESPO crude G2G via Lemigas + LPG; ESPO API 34–36° = Minas-compatible; ESPO discount vs Brent ~$10–15/bbl historically; blended subsidi gap impact ~Rp485–975/L reduction at 25–50% supply share — NOT reflected in M11 cost recovery unless `RUSSIA_CRUDE_DISCOUNT_USD` env var is set (default 0 = conservative).
