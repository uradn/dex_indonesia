...
1. Run all relevant macro tools in parallel to gather current data:
   ...
   - `bop_engine` — trade balance, current account, import growth, Greenspan-Guidotti ratio
   - `sovereign_risk_engine` — SBN yield, CDS, credit rating
   - `banking_stress_engine` — NPL, LDR, CAR, IndONIA spread, M2, real rate
   - `uln_engine` — DSR, GG ratio, ULN/GDP, hedging compliance
...
| 9 | Greenspan-Guidotti ratio | < 1.0 (FX reserves < short-term ULN) | uln / bop |
| 10 | ULN DSR % | > 25% (IMF threshold) | uln |
| 11 | SBN 10Y yield | > 7.5% | sovereign |
| 12 | CDS 5Y | > 300 bps | sovereign |

### Banking Crisis Indicators (threshold → signal)

| # | Indicator | Signal Fires When | Source |
|---|-----------|-------------------|--------|
| 13 | NPL gross % | > 3% (KLR early warning) | banking |
| 14 | LDR % | > 100% | banking |
| 15 | IndONIA-BI Spread | > 50 bps (approaching corridor ceiling) | banking |
| 16 | CAR % | < 14% | banking |
| 17 | IHPR YoY | < 0% (collateral deflation) | banking |
| 18 | ULN hedging compliance | < 70% (1997 mechanism trigger zone) | uln |
| 19 | GDP growth trend | Decelerating for 2+ quarters | regime |
| 20 | Inflation ROC | Accelerating while growth decelerating (= Q3 Stagflation regime) | regime |
| 21 | SRBI-IndONIA nexus | SRBI > 900T IDR AND IndONIA spread > 30bps | banking |

## Crisis Probability Classification

- **0–3 signals:** LOW — no crisis signal. Monitor normally.
- **4–7 signals:** MODERATE — elevated. Increase monitoring frequency.
- **8–12 signals:** HIGH — pre-crisis conditions. Escalate to ORANGE.
- **13+ signals:** CRITICAL — twin crisis risk. RED alert.

## BI Transmission Context

When IndONIA-BI spread is elevated, interpret within BI corridor:
- BI Rate currently at 5.25% (check sovereign_risk for latest)
- Corridor: DFR = BI Rate − 100bps (floor), LF Rate = BI Rate + 75bps (ceiling)
...
**Currency Crisis Signals:** X/12 active
**Banking Crisis Signals:** X/9 active
**Total Active:** X/21
